CREATE DATABASE `car_app_test`;

CREATE TABLE `car_app_test`.`user` (
...
CREATE TABLE `car_app_test`.`car_model` (
  `id` INT NOT NULL AUTO_INCREMENT,
...
-- DROP TABLE `car_app_test`.`car_model`;
CREATE TABLE `car_app_test`.`car_lot` (
  `id` INT NOT NULL AUTO_INCREMENT,
...
ALTER TABLE
  `car_app_test`.`car_lot`
ADD
  CONSTRAINT `user_car_lot_ownerId` FOREIGN KEY (`ownerId`) REFERENCES `car_app_test`.`user` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;

CREATE TABLE `car_app_test`.`car` (
  `id` INT NOT NULL AUTO_INCREMENT,
  ...
  CONSTRAINT `car_lot_car_car_lot_id` FOREIGN KEY (`carLotId`) REFERENCES `car_app_test`.`car_lot` (`id`) ON DELETE RESTRICT ON UPDATE CASCADE,
  CONSTRAINT `car_model_car_car_model_id` FOREIGN KEY (`carModelId`) REFERENCES `car_app_test`.`car_model` (`id`) ON DELETE RESTRICT ON UPDATE CASCADE
);

CREATE TABLE `car_app_test`.`car_payment` (
